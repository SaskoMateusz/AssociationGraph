...
public class Graph {
    ...
    static Graph BuildofMaps(List<Map<String, Integer>> associations, List<String> words) {
        ...
        for (int i = 0; i < words.size(); i++) {
            ...
            for (Map.Entry<String, Integer> association : associations.get(i).entrySet()) {
                name.addNeighbour(graph.verticesByName.get(association.getKey()),
    ...
    List<WeightPair<String>> getShortestPath(String source, String target) {
        ...
        while (!queue.isEmpty()) {
            ...
            for (WeightPair<Vertex> neighbor : poll.getTarget().getNeighbour()) {
    ...
    private boolean retrievePath(Vertex vertex, String target, List<WeightPair<String>> appendable) {
        if (vertex.getWord().equals(target)) {
            appendable.add(new WeightPair<>(target, 0.0));
            return true;
        }
        double myDist = distancesByName.get(vertex.getWord());
        distancesByName.put(vertex.getWord(), Double.POSITIVE_INFINITY);
        for (WeightPair<Vertex> neighbor : vertex.getNeighbour()) {
            if (distancesByName.get(neighbor.getTarget().getWord()) == myDist + neighbor.getWeight()) {
                if (retrievePath(neighbor.getTarget(), target, appendable)) {
                    appendable.add(new WeightPair<>(vertex.getWord(), neighbor.getWeight()));
                    return true;
                }
            }
        }
